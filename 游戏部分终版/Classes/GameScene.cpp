...
bool GameScene::init()
{
	...
	schedule(schedule_selector(GameScene::lay), 5.0f, kRepeatForever, 0);
...
void GameScene::addFarmer() {
	dir = true; //  use to change the direction of the player
//使用第一帧创建精灵
...
void GameScene::preloadMusic() {
	...
	audio->preloadEffect("music/母鸡下蛋.wav");
	audio->preloadEffect("music/游戏结束.wav");
	audio->preloadEffect("music/游戏开始.mp3");
}
//播放背景音乐
...
void GameScene::update(float f) {
	...
	if ((*eagleIt) != NULL) {
		...
		if ((*henIt) != NULL) {
			...
			if ((*eagleIt)->getPosition().getDistance(henPos) < 50) {
...
void GameScene::meet2(EventCustom * event) {
	//判断老鹰是否抓住母鸡并执行对应操作（若抓住母鸡数减一，老鹰飞回天空；若失败则老鹰飞回天空）
	//todo
	//判断游戏是否结束（母鸡个数是否为0）并执行对应操作
	if (hens.size() == 0) {
		SimpleAudioEngine::getInstance()->playEffect("music/gameover.wav", false);
		unschedule(schedule_selector(GameScene::update));
		this->getEventDispatcher()->removeAllEventListeners();
...
void GameScene::addCustomListener() {
	...
	auto meetListener2 = EventListenerCustom::create("meet2", CC_CALLBACK_1(GameScene::meet2, this));
	_eventDispatcher->addEventListenerWithFixedPriority(meetListener2, 1);
}
...
bool GameScene::onTouchBegan(Touch *touch, Event *unused_event) {
	...
	auto newp = Vec2(origin.x + 80, origin.y + 350);
	bool flag = false;
	auto it = eggs.begin();
	while (it != eggs.end()) {
		flag = false;
